    ...
    with record_function("DLRM forward"):
        if use_gpu:  # .cuda()
            ...
            if ndevices == 1:
                if sk_flag:
                    if X != None:
                        return wdl(X.to(device), lS_o, lS_i, test, train_time)
                    else:
                        return wdl(None, lS_o, lS_i, test, train_time)
                else:
                    ...
                    if use_adaptive_encoding:
                        ...
                        # for embedding tables whose sizes are > max_size, use Huffman Coding;
                        # otherwise, use the original sizes and index mappings.
                        for k, sparse_index_group_batch in enumerate(lS_i):
                            emb_index_group_batch = sparse_index_group_batch
                            emb_offset_group_batch = lS_o[k]
                            # print(f"emb_index_group_batch: {emb_index_group_batch}")
                            # input()
                            if use_huffman_coding and ln_emb[k] > capacity:
                                # print(f"use_huffman_coding and ln_emb[{k}]: {ln_emb[k]} > {capacity}")
                                ...
                                # print(f"before hashing emb_index_group_batch: {emb_index_group_batch}")
                                # input()
                                emb_index_group_batch %= capacity
                                # print(f"after hashing emb_index_group_batch: {emb_index_group_batch}")
                            # else:
                                # print(f"use original, ln_emb[{k}]: {ln_emb[k]} capacity: {capacity}")
...
class WDL_Net(nn.Module):
    ...
    # def create_emb(self, m, ln, weighted_pooling=None, max_lens=None, use_huffman_coding=False, use_hashing_trick=False):
    def create_emb(
        self,
        m: int,
        ln: List[int],
...
#        table_feature_map = {idx: idx for idx in range(len(train_data.counts))}
    ...
    if args.use_huffman_coding:
        # with open('huffman_coding_tensors_kdd12_01252024.pkl', 'rb') as file:
        # with open('huffman_coding_tensors_01252024.pkl', 'rb') as file:
